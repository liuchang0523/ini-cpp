                ...
                *prev_name = '\0';
				...
				if (*end == '=' || *end == ':') {
					*end = '\0';
					...
					///
					//在此处添加转换
					std::string value_str = utf8_to_gbk(std::string(value));
					std::string name_str = utf8_to_gbk(std::string(name));
					std::string section_str = utf8_to_gbk(std::string(section));

					/* Valid name[=:]value pair found, call handler */
					strncpy0(prev_name, name_str.c_str(), sizeof(prev_name));
					if (!HANDLER(user, section_str.c_str(),
						name_str.c_str(), value_str.c_str()) && !error)
						error = lineno;
            } else if (!error) {
